    ...
    def train(self, dataset):
        ...
        while True:
            ...
            for x, y in dataset.iterate_once(batch_size):
                ...
                target_value = nn.as_scalar(y)
                
                if result != target_value:
                    update_value = nn.Constant(target_value * x.data)
                    self.w.update(update_value, 1)
                    change_flag = True
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        final = nn.AddBias(nn.Linear(second, self.w3),self.b3)
        return final
...
class DigitClassificationModel(object):
    ...
    def run(self, x):
        ...
        final = nn.AddBias(nn.Linear(third, self.w4),self.b4)
        return final
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        """
        "*** YOUR CODE HERE ***"
        h = nn.ReLU(nn.AddBias(nn.Linear(xs[0], self.initial_w), self.initial_b))
        for char in xs[1:]:
            h = nn.ReLU(nn.AddBias(nn.Add(nn.Linear(char, self.x_w), nn.Linear(h, self.h_w)), self.b))
