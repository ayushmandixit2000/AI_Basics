...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            hitsWall = self.walls[nextx][nexty]
            if hitsWall == False:
                corners_left = tuple(c for c in state[1] if c != (nextx, nexty))  # filters out the corners in state[1] that are not equal to (nextx, nexty)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    "*** YOUR CODE HERE ***"
    # Similar to function definition in HW 1
    x, y = state[0]
    corners_left = state[1]
    max_d = 0
    for c in corners_left:
        cur_d = abs((x-c[0]))+ abs((y-c[1]))
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in foods:
        cur_d = mazeDistance(food, state[0], problem.startingGameState)
        max_d = max(max_d, cur_d)
