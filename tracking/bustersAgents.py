...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        def pacmanDist(ghostPosition):
            return distance(pacmanPosition, ghostPosition)

        closest = min(ghostPositions, key=pacmanDist)

        def distFromNearestGhost(action):
            newPacmanPosition = Actions.getSuccessor(pacmanPosition, action)
            return distance(newPacmanPosition, closest)

        greedy = min(legalActions, key=distFromNearestGhost)
        return greedy
        "*** END YOUR CODE HERE ***"
