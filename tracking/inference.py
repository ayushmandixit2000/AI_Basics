...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict = {PAC: [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)], 
                           ...
                           OBS0: [x for x in range(X_RANGE+Y_RANGE+MAX_NOISE-1)], 
                           OBS1: [x for x in range(X_RANGE+Y_RANGE+MAX_NOISE-1)] 
                           }
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            curr, joinedFactor = joinFactorsByVariable(curr, var)
            if len(joinedFactor.unconditionedVariables()) == 1:
                continue
            eliminated = eliminate(joinedFactor, var)
            curr.append(eliminated)
        temp = joinFactors(curr)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        sumT = .0
        for key in self:
            if sumT <= probability < sumT + self[key]:
                return key
            sumT += self[key]
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance is not None and ghostPosition == jailPosition:
            return 0
        if noisyDistance is not None and ghostPosition != jailPosition:
            pacmanToGhostDistance = manhattanDistance(pacmanPosition, ghostPosition)
            return busters.getObservationProbability(noisyDistance, pacmanToGhostDistance)
        return 0
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        currentPacmanPos = gameState.getPacmanPosition()
        currentJailPos = self.getJailPosition()
        currentBeliefs = self.beliefs 

        for ghostPos in self.allPositions:
    
            probNoisyGivenActual = self.getObservationProb(
            observation, currentPacmanPos, ghostPos, currentJailPos)
            currentBeliefs[ghostPos] *= probNoisyGivenActual
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        posJail = self.getJailPosition()
        beliefDist = self.getBeliefDistribution()
        for ghostPos in beliefDist:
            beliefDist[ghostPos] *= self.getObservationProb(observation,
                                                    posPacman,
                                                    ghostPos, posJail)
        if beliefDist.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = random.choices(list(beliefDist.keys()), k=self.numParticles,
                                    weights=beliefDist.values())
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for currentParticle in self.particles:
            newPosDist = self.getPositionDistribution(gameState, currentParticle)
            sampledNextPos = newPosDist.sample()
            updatedParticles.append(sampledNextPos)
